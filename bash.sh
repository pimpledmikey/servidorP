...
check_server_connection() {
    nc -z 189.196.89.74 36
    return $?
}

check_server_connectionTootal() {
    nc -z 187.189.75.122 36
    return $?
}
...
if ! check_server_connection; then
    send_email
    echo "Se ha enviado un correo electrónico debido a la falta de conexión al servidor Megabcable."
fi



# Verificar la conexión al servidor
if ! check_server_connectionTootal; then
    send_email
    echo "Se ha enviado un correo electrónico debido a la falta de conexión al servidor TotalPlay."
fi
